...
def _create_session_if_needed(session_id):
    with global_data_lock:
        if session_id not in global_data['sessions']:
            global_data['sessions'][session_id] = Session(session_id)

def _get_remote_job_handler(session_id, job_handler_name, uri):
    with global_data_lock:
        session = global_data['sessions'][session_id]
        if job_handler_name not in session._job_handlers:
            session._job_handlers[job_handler_name] = hi.RemoteJobHandler(uri=uri)
        return session._job_handlers[job_handler_name]
...
def create_hither_job():
    ...
    _create_session_if_needed(session_id)
    ...
    if a['type'] == 'local':
        with global_data_lock:
            jh = global_data['local_job_handlers'][job_handler_name]
    elif a['type'] == 'remote':
        jh = _get_remote_job_handler(session_id=session_id, uri=a['uri'], job_handler_name=job_handler_name)
    else:
    ...
    with global_data_lock:
        with ka.config(**kachery_config):
            with hi.Config(**hither_config):
                ...
                job_id = job._job_id
                global_data['sessions'][session_id]._jobs_by_id[job_id] = job
...
def hither_job_wait():
    ...
    global global_data
    with global_data_lock:
        assert session_id in global_data['sessions'], f'No session: {session_id}'
        if job_id not in global_data['sessions'][session_id]._jobs_by_id:
            return dict(
                error=True,
                error_message=f'No job with id: {job_id}',
                runtime_info=None
            )
        job: hi.Job = global_data['sessions'][session_id]._jobs_by_id[job_id]
    timer = time.time()
    while True:
        with global_data_lock:
            try:
...
def hither_job_cancel():
    ...
    with global_data_lock:
        assert session_id in global_data['sessions'], f'No session: {session_id}'
        assert job_id in global_data['sessions'][session_id]._jobs_by_id, f'No job with id: {job_id}'
        job: hi.Job = global_data['sessions'][session_id]._jobs_by_id[job_id]
    job.cancel()
    return dict()
