    ...
    def __init__(self, file_path, electrical_series_name='ElectricalSeries'):
        ...
        with NWBHDF5IO(self._path, 'r') as io:
            ...
            if hasattr(es, 'timestamps') and es.timestamps:
                # jfm adjusted this calc of sampling freq
                n1 = len(es.timestamps)
                n2 = min(n1, 1000)
                freq1 = (n1 - 1) / (es.timestamps[n1 - 1] - es.timestamps[0])
                freq2 = (n2 - 1) / (es.timestamps[n2 - 1] - es.timestamps[0])
                if abs(freq1 - freq2) > 100:
                    raise Exception(f'Unable to determine sampling rate. Perhaps timestamp array is not continuous. {freq1} <> {freq2}')
                self._sampling_frequency = freq1
