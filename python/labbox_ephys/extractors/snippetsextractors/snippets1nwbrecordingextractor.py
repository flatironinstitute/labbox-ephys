...
class Snippets1NwbRecordingExtractor(se.RecordingExtractor):
    extractor_name = 'Snippets1NwbRecordingExtractor'
    is_writable = False
    def __init__(self, *, snippets_nwb_uri: str, nwb_object_prefix: str, p2p: bool=False):
        ...
        with pynwb.NWBHDF5IO(path=self._snippets_nwb_path, mode='r') as io:
            nwbf = io.read()
            self._sampling_frequency: float = np.array(nwbf.get_scratch(f'{nwb_object_prefix}_sampling_frequency'))[0]
