...
@hi.function('createjob_get_recording_info', '0.1.2', register_globally=True)
...
def estimate_noise_level(recording: se.RecordingExtractor):
    ...
    X = cast(np.ndarray, recording.get_traces(channel_ids=[int(id) for id in recording.get_channel_ids()], start_frame=start_frame, end_frame=end_frame))
...
@hi.function('createjob_download_recording', '0.1.0', register_globally=True)
def createjob_download_recording(labbox, recording_object):
    return download_recording.run(recording_object=recording_object)

@hi.function('createjob_recording_is_downloaded', '0.1.0', register_globally=True)
