...
class CanvasPainter {
    ...
    pen() {
        return shallowClone(this.#pen)
    }
    font() {
        return shallowClone(this.#font)
    }
    brush() {
        return shallowClone(this.#brush)
    }
    setPen(pen: Pen) {
        this.#pen = pen
    }
    ...
    clear(): void {
        this._clearRect(0, 0, this.#canvasLayer.width(), this.#canvasLayer.height());
    }
    _clearRect(x: number, y: number, W: number, H: number) {
        this.#context2D.clearRect(x, y, W, H);
    }
    clearRect(x: number, y: number, W: number, H: number) {
        // why different from _clearRect??
    ...
    ctxTranslate(dx: number | [number, number], dy: number | undefined) {
        if (dy === undefined) {
            if (typeof dx === 'number') {
    ...
    coordsToPix(x: number | [number, number], y: number | undefined): [number, number] {
        if (y === undefined) {
            if (typeof x === 'number') {
                throw Error('unexpected');
            }
    ...
    fillRect(x: number | [number, number, number, number], y: number | Brush, W: number | undefined, H: number | undefined, brush: Brush | undefined) {
        if (typeof x === 'object') {
            let a = x;
            if (typeof y !== 'object') {
                throw Error('unexpected');
    ...
    transformXY(x: number | [number, number], y: number | undefined): [number, number] {
        if (y === undefined) {
            if (typeof x === 'number') {
                throw Error('unexpected')
            }
