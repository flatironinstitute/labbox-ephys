...
def createjob_get_sorting_unit_snippets(labbox, recording_object, sorting_object, unit_id, max_num_snippets):
...
@hi.function('get_sorting_unit_snippets', '0.1.3')
@hi.container('docker://magland/labbox-ephys-processing:0.3.19')
@hi.local_modules(['../../../python/labbox_ephys'])
def get_sorting_unit_snippets(snippets_h5, unit_id, max_num_snippets):
    import h5py
    ...
    with h5py.File(h5_path, 'r') as f:
        ...
        unit_spike_train = np.array(f.get(f'unit_spike_trains/{unit_id}'))[:max_num_snippets]
        unit_waveforms = np.array(f.get(f'unit_waveforms/{unit_id}/waveforms'))[:max_num_snippets, :, :]
